...
int main()
{
    ...
    const physx::PxU32 size = 30;
    physx::PxTransform t( physx::PxVec3( 0 ) );

    physx::PxRigidDynamic* actors[ size * size ];
    ...
    while ( 1 )
    {
        ...
        physx::PxVec3 origin = physx::PxVec3( -10, 0, 0 );
        physx::PxVec3 unitdir = physx::PxVec3( 1, 0, 0 ).getNormalized();
        physx::PxReal maxDistance = physx::PxReal( 3 );
        bool status = mScene->raycast( origin, unitdir, maxDistance, hit );
        if ( status )
        {
            std::cout << hit.block.position.x << ' ' << hit.block.position.y << ' ' << hit.block.position.z << '\n';
        }
    }
}